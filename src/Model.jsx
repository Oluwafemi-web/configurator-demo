...
export default function Model({
  ...
  const chairTexture = chairTexturePath
  const pillowTexture = pillowTexturePath
  const feetTexture = feetTexturePath

  // Clone materials ONCE so instances don’t bleed into each other
  const sofaMaterials = useMemo(() => {
    return {
      chair: materials.Chair?.clone(),
      pillow: materials.Pillow?.clone(),
      feet: materials.Feet?.clone(),
    }
  }, [materials])
  console.log(sofaMaterials)

  // Apply textures declaratively
  if (chairTexture && sofaMaterials.chair) {
    sofaMaterials.chair.map = chairTexture
    sofaMaterials.chair.metalness = 0
    sofaMaterials.chair.roughness = 0.9
  }

  if (pillowTexture && sofaMaterials.pillow) {
    sofaMaterials.pillow.map = pillowTexture
    sofaMaterials.pillow.metalness = 0
    sofaMaterials.pillow.roughness = 0.9
  }

  if (feetTexture && sofaMaterials.feet) {
    sofaMaterials.feet.map = feetTexture
    sofaMaterials.feet.metalness = 0.8
    sofaMaterials.feet.roughness = 0.2
  }

  return (
    <group position dispose={null}>
      {/* Seat */}
      <mesh geometry={nodes.Jump_Center_seat.geometry} material={materials.plastic} />

      {/* Back */}
      <mesh geometry={nodes.Jump_Center_back.geometry} material={materials.plastic} />


      {/* Feet */}
      <mesh geometry={nodes.Jump_Center_feet.geometry} material={materials.plastic} />
